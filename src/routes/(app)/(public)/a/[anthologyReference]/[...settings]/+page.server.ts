...
import {
  findOneAnthologyByReference,
  findOneStoryByReference
...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  if (!anthology)
    error(
      404,
      `The anthology '${anthologyReference}' was not found. It may not exist, or may not be published, yet.`
    );
  ...
  for (const [storyIndex, { reference: storyReference }] of anthology.stories.entries()) {
    const story = await findOneStoryByReference(clientId, storyReference, language);
    if (!story)
      error(
        404,
        `The story '${storyReference}' was not found. It may not exist, or may not be published, yet.`
      );
    ...
    const players = story?.parts?.map((part, index) => ({
      ...
      start:
        part.background?.start && part.background?.duration
          ? part?.background?.start * part.background?.duration
          : undefined,
      end:
        part.background?.end && part.background?.duration
          ? part?.background?.end * part.background?.duration
          : undefined,
