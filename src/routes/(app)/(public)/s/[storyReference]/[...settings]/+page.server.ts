...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const story = await findOneStoryByReference(clientId, params.storyReference, language);
  if (!story)
    error(
      404,
      `The story '${params.storyReference}' was not found. It may not exist, or may not be published, yet.`
    );
