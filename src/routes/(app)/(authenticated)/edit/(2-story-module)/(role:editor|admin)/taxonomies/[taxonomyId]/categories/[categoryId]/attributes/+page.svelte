<script lang="ts">
	...
	const taxonomyBasePath = `/edit/taxonomies/${data.category.taxonomyId}`;
	// svelte-ignore state_referenced_locally
	const endpoint = `/api/taxonomies/${data.category.taxonomyId}/categories/${data.category.id}/attributes`;
	...
	const attributeOptions = () =>
		data.attributeOptions.map((attribute) => ({
			title:
				translateLocalizedField(attribute.name as Translatable, UI.language) ?? attribute.slug,
			value: attribute.id,
...
<svelte:head>
	<title>Edit category attributes</title>
</svelte:head>
		...
		menus={[
			[
				...
				{ label: 'Categories', url: `${taxonomyBasePath}/categories` },
				{
					isTrigger: true,
					label: 'Attributes',
					url: `${taxonomyBasePath}/categories/${data.category.id}/attributes`
				}
			]
		]}
	/>
