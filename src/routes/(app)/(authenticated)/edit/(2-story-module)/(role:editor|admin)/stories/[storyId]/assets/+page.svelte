<script lang="ts">
	...
	const stillOptions = () =>
		relations.allAvailableStills.map((asset) => ({
			title: asset.title ?? '?',
			summary: asset.summary,
			value: asset.asset
		}));
	...
	const createGrid = (rows: () => AssetRow[], type: 'still' | 'video' | 'announcement' | 'quiz') =>
		useDataGrid<AssetRow>({
				...
				if (column.id === 'asset') {
					if (type === 'still') {
						return {
							...column,
							meta: {
								cell: { variant: 'relation-select-single', options: stillOptions() }
							}
						};
					} else if (type === 'video') {
						return {
			...
			onRowsDelete: async (removedrows: AssetRow[], rowIndices: number[]) => {
				...
				if (type === 'still') {
					stillRows = stillRows.filter((row, index) =>
...
<div class="mx-auto mt-4 w-full max-w-6xl space-y-6 px-4 pb-10">
	<Item.Root variant="outline">
		<Item.Content>
			...
			<Item.Description class="line-clamp-1">{story.slug}</Item.Description>
		</Item.Content>
	</Item.Root>

	<section class="space-y-3">
		<h2 class="text-lg font-semibold">Stills</h2>
		<div role="toolbar" aria-orientation="horizontal" class="flex items-center justify-between">
			<DataGridKeyboardShortcuts enableSearch={!!stillGrid.searchState} />
			<div class="flex w-full items-center gap-1">
				<DataGridFilterMenu table={stillGrid.table} />
				<DataGridSortMenu table={stillGrid.table} />
				<DataGridRowHeightMenu table={stillGrid.table} />
				<DataGridViewMenu table={stillGrid.table} />
			</div>
		</div>
		<DataGrid {...stillGrid} height={gridHeight} />
	</section>
