...
const partitionAvailableAssets = (rows: AssetRow[]) => {
  ...
  const allAvailableStills: (Extract<AssetRow, { type: 'still' }> & {
    title: string;
    summary: string | null;
  })[] = [];
  const allAvailableAnnouncementTemplates: (Extract<AssetRow, { type: 'announcement' }> & {
    title: string;
    summary: null;
  })[] = [];
  const allAvailableQuizTemplates: (Extract<AssetRow, { type: 'quiz' }> & {
    title: string;
    summary: string;
  })[] = [];
    ...
    switch (asset.type) {
      case 'still':
        allAvailableStills.push({
          ...asset,
          title: asset.image?.filename ?? asset.color ?? 'Untitled still',
          summary: asset.style
        });
        break;
      case 'video':
...
export const load: PageServerLoad = async ({ locals, params, fetch }) => {
  ...
  const storyAssets: AssetRow[] = storyAssetsRes.ok
    ? ((await storyAssetsRes.json()) as AssetRow[])
    : [];
