...
const clientSchema = z.object({
	...
	locales: z.enum(Language).array().min(1),
	administrationEmail: z.string().trim().email().nullable(),
	logo: z
	...
	plausibleDomain: z.string().trim().min(1).nullable(),
	authenticationMethods: z.enum(ClientAuthenticationMethod).array().min(1),
	accessTokenKey: z.string().trim().min(1),
	redirectAuthorized: z.string().trim().min(1).nullable(),
	redirectUnauthorized: z.string().trim().min(1).nullable()
});

export const clientCreateSchema = clientSchema.extend({
	locales: clientSchema.shape.locales.default([Language.English]),
	...
	administrationEmail: clientSchema.shape.administrationEmail.optional(),
