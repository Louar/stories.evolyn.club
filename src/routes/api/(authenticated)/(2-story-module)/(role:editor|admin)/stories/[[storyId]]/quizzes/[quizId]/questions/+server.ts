...
import { findOneQuizById } from '$lib/db/repositories/2-story-module';
...
const questionSchema = z.object({
  id: z.string().optional(),
  answerTemplateSlug: z.literal('select-single'),
  order: z.number(),
  title: z.preprocess(
    formObjectPreprocessor,
    translatableValidator
      ...
      if (answerGroup) {
        const answerGroupResult = await trx
          ...
          .values({
            ...
            // slug: `answer-group-${crypto.randomUUID().toString().slice(0, 8)}`,
            // name: `Answer Group ${answerGroup.id}`,
            doRandomize: answerGroup.doRandomize
          })
          .onConflict((oc) =>
            oc.columns(['id']).doUpdateSet({
              // slug: `answer-group-${answerGroup.id}`,
              // name: `Answer Group ${answerGroup.id}`,
              doRandomize: answerGroup.doRandomize
            })
          )
