...
const listStillsAllStories = async (args: {
	clientId: string;
	permittedStoryIds: string[] | null;
}): Promise<Extract<AssetRow, { type: 'still' }>[]> => {
	let qb = db
		.selectFrom('stillAvailableToStory as link')
		.distinctOn('link.stillId')
		.innerJoin('still', 'still.id', 'link.stillId')
		.innerJoin('story', 'story.id', 'link.storyId')
		.where('story.clientId', '=', args.clientId)
		.orderBy('link.stillId')
		.select(['link.id as id', 'still.id as asset', 'still.color', 'still.image', 'still.style']);

	if (args.permittedStoryIds !== null) qb = qb.where('story.id', 'in', args.permittedStoryIds);

	const rows = await qb.execute();
	return rows.map((row) => ({ type: 'still', ...row }));
};
...
const listVideosForStory = async (
	storyId: string
): Promise<Extract<AssetRow, { type: 'video' }>[]> => {
	const rows = await db
		.selectFrom('videoAvailableToStory as link')
...
const listStillsForStory = async (
	storyId: string
): Promise<Extract<AssetRow, { type: 'still' }>[]> => {
	const rows = await db
		.selectFrom('stillAvailableToStory as link')
...
const listQuizzesForStory = async (
	storyId: string
): Promise<Extract<AssetRow, { type: 'quiz' }>[]> => {
	const rows = await db
		.selectFrom('quizTemplateAvailableToStory as link')
...
const findInsertedAssetRow = async (storyId: string, type: AssetType, linkId: string) => {
	...
	const row = rows.find((row) => row.id === linkId);
	if (!row) throw error(500, 'Inserted availability link could not be loaded');
	return row;
};
	...
	switch (type) {
		case 'still': {
			const asset = await db
				.selectFrom('still')
				.where('still.id', '=', assetId)
				.select('still.id')
				.executeTakeFirst();
			if (!asset) throw error(404, 'Still not found');
			return;
		}
		case 'video': {
			const asset = await db
				.selectFrom('video')
				.where('video.id', '=', assetId)
				.select('video.id')
				.executeTakeFirst();
...
const ensureSourceAccess = async (args: {
	...
	if (args.permittedStoryIds.length === 0)
		throw error(403, 'You are not allowed to use this asset');

	switch (args.type) {
		case 'still': {
			const exists = await db
				.selectFrom('stillAvailableToStory as sourceLink')
				.innerJoin('story as sourceStory', 'sourceStory.id', 'sourceLink.storyId')
				.where('sourceStory.clientId', '=', args.clientId)
				.where('sourceLink.stillId', '=', args.assetId)
...
export const GET: RequestHandler = async ({ locals, params, url }) => {
	...
	if (!parsedQuery.success)
		return json({ errors: z.flattenError(parsedQuery.error).fieldErrors }, { status: 422 });
		...
		switch (type) {
			case 'still': {
				const inserted = await db
					...
					.values({ storyId, stillId: assetId })
					.returning('id')
