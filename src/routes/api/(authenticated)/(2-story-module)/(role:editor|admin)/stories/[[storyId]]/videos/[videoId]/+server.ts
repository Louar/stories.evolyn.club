...
import { findOneVideoById } from '$lib/db/repositories/2-story-module';
import { formObjectPreprocessor, translatableMediaValidator } from '$lib/db/schemas/0-utils';
...
const videoSchema = z.object({
	name: z.string().min(1),
	source: z.preprocess(formObjectPreprocessor, translatableMediaValidator),
	thumbnail: z.preprocess(formObjectPreprocessor, translatableMediaValidator.nullable()),
...
export const DELETE = (async ({ locals, params }) => {
	...
	await db.deleteFrom('video').where('id', '=', params.videoId).executeTakeFirstOrThrow();
