...
export const InitClientUserModule: Migration = {
	...
	async up(db: Kysely<any>) {
		...
		await db.schema
			...
			.addColumn('is_findable_by_search_engines', 'boolean', (col) =>
		...
		await db.schema
			...
			.addColumn('phone_confirmed', 'boolean', (col) => col.defaultTo(false).notNull())
			...
			.addUniqueConstraint('unique_user_email_per_client', ['client_id', 'email'])
			.execute();
		await db.schema
			.alterTable('client')
			.addColumn('created_by', 'uuid', (col) => col.references('user.id').onDelete('set null'))
			...
			.execute();
		await db.schema
			.alterTable('client_media')
			.addColumn('created_by', 'uuid', (col) => col.references('user.id').onDelete('set null'))
			.addColumn('updated_by', 'uuid', (col) => col.references('user.id').onDelete('set null'))
			.execute();

		// Create the ClientApiKey table
		await db.schema
			.createTable('client_api_key')
			.ifNotExists()
			.addColumn('id', 'uuid', (col) =>
				col
					.primaryKey()
			...
			.addColumn('name', 'text', (col) => col.notNull())
			.addColumn('secret', 'text', (col) => col.unique().notNull())
			...
			.addColumn('updated_by', 'uuid', (col) => col.references('user.id').onDelete('set null'))
			.addUniqueConstraint('unique_client_api_key_name_per_client', ['client_id', 'name'])
			.execute();

		// Create the UserMedia table
		await db.schema
			.createTable('user_media')
			.ifNotExists()
			.addColumn('id', 'uuid', (col) =>
				col
					.primaryKey()
		...
		await db.schema
			...
			.addColumn('license_id', 'uuid', (col) =>
				col.references('license.id').onDelete('cascade').notNull()
			)
			.addColumn('user_id', 'uuid', (col) =>
				col.references('user.id').onDelete('cascade').notNull()
			)
	...
	async down(db: Kysely<any>) {
		...
		await db.schema.dropTable('client_api_key').ifExists().execute();
		await sql`DROP TRIGGER IF EXISTS created_user ON "user";`.execute(db);
