import { DEFAULT_CLIENT_SLUG, DEFAULT_USER_EMAIL } from '$app/env/private';
...
export const DummyDataStoryQuizOfCities = async (
  ...
  await db.transaction().execute(async (trx) => {
    ...
    if (!userId?.length)
      userId = (
        await trx
          ...
          .where('client.slug', '=', DEFAULT_CLIENT_SLUG)
          .where('user.email', '=', DEFAULT_USER_EMAIL)
          .select('user.id')
          .executeTakeFirstOrThrow()
      ).id;
    ...
    const agPlayPauseQ1 = await trx
      ...
      .values({ slug: 'play-pause-q1', name: 'Yes / No 1', doRandomize: true })
      .returning('id')
      .executeTakeFirstOrThrow();

    const agPlayPauseQ2 = await trx
      .insertInto('quizQuestionTemplateAnswerGroup')
      .values({ slug: 'play-pause-q2', name: 'Yes / No 2' })
      .returning('id')
      .executeTakeFirstOrThrow();
    ...
    const agCityLuzernQ1 = await trx
      .insertInto('quizQuestionTemplateAnswerGroup')
      .values({ slug: 'city-luzern-q1', name: 'City options: Luzern', doRandomize: false })
      .returning('id')
      .executeTakeFirstOrThrow();
