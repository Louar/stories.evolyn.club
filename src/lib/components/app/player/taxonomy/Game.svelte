<script lang="ts">
	...
	function clickMapFeature(feature: TopoGeometry): GuessResult {
		...
		if (correct) {
			...
			showFeedback({
				correct: true,
				title: m.taxonomy_map_correct_title(),
				description: m.taxonomy_map_correct_description({
					targetName: currentRound.target.name,
					referencedName: currentRound.target.referencedName
				})
			});
			return 'correct';
		}
		...
		showFeedback({
			...
			description: m.taxonomy_map_wrong_description()
		});
		return 'wrong';
	}
...
>
	{#if feedback}
		{#key feedback}
			<section
				...
				aria-label={m.taxonomy_submission_result()}
					...
					>
						{#if feedback.correct}
							<Button type="button" size="lg" class="w-full text-lg font-black" onclick={nextRound}>
								{currentRoundIndex === playableRounds.length - 1
									? m.taxonomy_show_results()
									: m.taxonomy_next_round()}
							</Button>
						{/if}
					</RoundFeedback>
				</div>
	...
	{#if playableRounds.length === 0}
		<section
			...
			role="alert"
		>
			<h2 class="text-2xl font-black">{m.taxonomy_no_playable_rounds_title()}</h2>
			<p class="mt-2 text-game-text-muted">
				{m.taxonomy_no_playable_rounds_description()}
			</p>
		</section>
	{:else if finished}
		<section
			...
			aria-live="polite"
		>
			<p class="text-sm font-bold tracking-wide text-game-text-muted uppercase">
				{m.taxonomy_game_complete()}
			</p>
			<h2 class="mt-2 text-3xl font-black">
				{m.taxonomy_completed_correct({ completed, total: playableRounds.length })}
			</h2>
			<p class="mt-2 text-game-text-muted">{m.taxonomy_mistakes_count({ count: mistakes })}</p>
		</section>
	{:else if currentRound}
		...
		<section
			...
			aria-label={m.taxonomy_game_status()}
		...
		>
			...
			>
				{#if playableRounds.length !== goal}
					<span
						aria-label={m.taxonomy_round_progress({
							current: currentRoundIndex + 1,
							total: playableRounds.length
						})}
				...
				<span
					class="rounded-md px-1.5 py-0.5 font-extrabold"
					aria-label={m.taxonomy_time_aria({ time: formatTime(timeMs) })}
					>{m.taxonomy_time()} {formatTime(timeMs)}</span
				>
			</div>
			...
			{#if arrowRotation !== undefined}
				<div
					...
					aria-label={m.taxonomy_direction_hint()}
