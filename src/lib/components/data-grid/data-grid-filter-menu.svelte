<script lang="ts" generics="TData">
	...
	} from '$lib/components/data-grid/data-grid-filters.js';
	import type {
		CellSelectOption,
		FilterOperator,
		FilterValue,
	...
	const { columnLabels, columns, columnVariants } = $derived.by(() => {
		...
		for (const column of table.getAllColumns()) {
			...
			const variant = column.columnDef.meta?.cell?.variant ?? 'text-short';
	...
	function onFilterAdd() {
		...
		const variant = columnVariants.get(firstColumn.value) ?? 'text-short';
	...
	function getColumnOptions(columnId: string): CellSelectOption[] {
		...
		if (cellVariant?.variant === 'select-single' || cellVariant?.variant === 'select-multiple') {
		...
		{#if filterItems.length > 0}
			...
			>
				{#each filterItems as filter, index (filter.id)}
					{@const variant = columnVariants.get(filter.id) ?? 'text-short'}
										...
										<CommandGroup>
											{#each columns as col (col.value)}
												<CommandItem
													value={col.value}
													onSelect={() => {
														const newVariant = columnVariants.get(col.value) ?? 'text-short';
								...
								{:else if (variant === 'select-single' || variant === 'select-multiple') && selectOptions.length > 0}
									...
									{:else}
										...
										<Popover>
											...
											<PopoverContent align="start" class="w-50 p-0">
												<Command>
													<CommandInput placeholder="Search options..." />
													<CommandList>
														<CommandEmpty>No options found.</CommandEmpty>
