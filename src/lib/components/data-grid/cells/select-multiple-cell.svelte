<script lang="ts" generics="TData">
	...
	const options = $derived(cellOpts?.variant === 'select-multiple' ? cellOpts.options : []);
	...
	const displayItems = $derived(
		selectedValues
			.map((val) => ({
				value: val,
				title: options.find((opt) => opt.value === val)?.title ?? val
			}))
	...
	const badgeOverflow = useBadgeOverflow(() => ({
		...
		lineCount: lineCount,
		cacheKeyPrefix: 'select-multiple'
	}));
			...
			>
				...
				>
					...
					<CommandList class="max-h-full">
						...
						<CommandGroup class="max-h-75 scroll-py-1 overflow-x-hidden overflow-y-auto">
							{#each options as option (option.value)}
								{@const isItemSelected = selectedValues.includes(option.value)}
								<CommandItem value={option.title} onSelect={() => handleValueChange(option.value)}>
									<CheckIcon
										class={cn(
