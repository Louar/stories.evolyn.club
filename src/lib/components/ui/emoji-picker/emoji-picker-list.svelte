...
<script lang="ts">
	...
	import i18n from '@emoji-mart/data/i18n/nl.json';
	...
	let {
		ref = $bindable(null),
		emptyMessage = i18n.search_no_results_2,
		class: className,
		...rest
...
<Command.List bind:ref class={cn('relative h-[200px]', className)} {...rest}>
	...
	{#each emojiData.categories as category (category.id)}
		...
		{#if emojis.length > 0}
			<CommandPrimitive.Group>
				<CommandPrimitive.GroupHeading class="px-2 py-1 text-xs text-muted-foreground">
					{i18n.categories[category.id as keyof typeof i18n.categories]}
				</CommandPrimitive.GroupHeading>
				<CommandPrimitive.GroupItems class="grid grid-cols-6 px-2">
