<script lang="ts">
	...
	interface Props {
		...
		/**
		 * Bindable slug to the underlying MapLibre map instance.
	...
	function getViewport(mapInstance: Map): MapViewport {
	...
	onMount(() => {
		...
		if (browser) {
			...
			const updateTheme = () => {
				...
				// Only use document theme if set, otherwise fall back to system pslug
			...
			// Also watch for system pslug changes
			mediaQuery = window.matchMedia('(prefers-color-scheme: dark)');
			handleSystemChange = (e: MediaQueryListEvent) => {
				// Only use system pslug if no document class is set
				if (!getDocumentTheme()) {
...
<div bind:this={mapContainer} class={cn('relative z-0 h-full w-full', className)}>
	{#if !isLoaded || loading}
		<div
